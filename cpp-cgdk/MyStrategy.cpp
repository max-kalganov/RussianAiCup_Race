...
{
	double sX, sY;
	sX = (self.getMass()*self.getSpeedX() * self.getSpeedX()) / (2 * 1000 * game.getCarCrosswiseMovementFrictionFactor()) - 200;//смотрю рассто€ние по ’, которое проеду до полной остановки на тормозах
...
{
    static int best_way[25][2][29]; // best_way[кол-во путей] [в 0 - ’-ы, в 1 - ”-и] [маршрут от какой-то точки до ключ. тайла] 
  ...
  ///   ¬сЄ до этого момента заполн€ет массив идеального пути . Ётой части на заполнение массива нужно world.getWaypoints().size() - тиков   
	if (world.getTick() > 300)
	{
		...
		createVectorAndPovorot(n, self, axis, world, best_way[0], best_way[1], best_way[2], game);//создаем вектор оси.ќсталось только определить его направление(ли бо + либо - ).Ёто € сделал, но не сохранил.потом добавлю.Ёто считай что сделано 
		double x, y, i, j;
		int znak;
		i = axis[0];
		j = axis[1];
